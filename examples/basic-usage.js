...
const largeDataset = generatePagination({ current: 500, max: 2000 });
...
const blog = generatePagination({ current: 15, max: 100, pad: 1 });
...
const customGap = generatePagination({
  current: 75,
  max: 200,
  pad: 2,
  gapSymbol: '-'
...
const firstPage = generatePagination({ current: 1, max: 100 });
console.log('First page:', firstPage.pages.slice(0, 10).join(' '), '...');

const lastPage = generatePagination({ current: 100, max: 100 });
console.log('Last page:', lastPage.pages.slice(-10).join(' '));

const smallDataset = generatePagination({ current: 3, max: 5 });
...
const advancedCustom = generatePagination({
  current: 150,
  max: 1000,
  pad: 1,
  gapSymbol: '...',
...
const deepPage = generatePagination({ current: 5000, max: 10000 });
...
const minimal = generatePagination({ current: 42, max: 100 });
...
const compact = generatePagination({ current: 50, max: 200, pad: 0 });
...
const extended = generatePagination({ current: 50, max: 200, pad: 5 });
console.log('Extended:', extended.pages.join(' '));

// Display result structure for documentation
console.log('\n=== Result Structure ===');
const example = generatePagination({ current: 10, max: 50 });
